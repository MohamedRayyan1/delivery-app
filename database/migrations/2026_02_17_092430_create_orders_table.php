...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('orders', function (Blueprint $table) {
            ...
            $table->decimal('subtotal', 10, 2); //سعر الطلب صافي
            $table->decimal('delivery_fee', 10, 2); //صافي تكلفة التوصيل
            $table->decimal('discount_amount', 10, 2)->default(0); // قيمة الخصم
            $table->decimal('grand_total', 10, 2); // سعر الكلي ( شقد لازم يدفع الزبون)

            // النسب المستخدمة وقت الطلب (Snapshots) للحساب لاحقاً
            // ملاحظة: سنفترض أن هذه نسب مئوية (مثلاً 10.00%)
            $table->decimal('applied_restaurant_commission', 5, 2)->default(0); // نسبة المطعم
            $table->decimal('applied_driver_share', 5, 2)->default(100); // نسبة السائق
